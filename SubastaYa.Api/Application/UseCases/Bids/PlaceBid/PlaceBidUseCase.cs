...
public class PlaceBidUseCase
{
    ...
    public async Task ExecuteAsync(int auctionId, string bidderId, PlaceBidRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        // 1. Retener saldo del nuevo postor
        await _walletService.RetainBidAmountAsync(bidderId, request.Amount, cancellationToken);

        // 2. Liberar saldo del postor anterior (si existe)
        if (highestBid != null)
        {
            await _walletService.RefundBidAmountAsync(highestBid.BidderId, highestBid.Amount, cancellationToken);
        }
